...
static bool g_goal_active = false;
static int g_result_output = -1;
static int g_fdbk = -1;
...
{
    ros::init(argc, argv, "timer_action_client_node");
    ros::NodeHandle n;
    ...
    {
        std::cout << "enter a desired timer value, in seconds (0 to abort, < 0 to quit): ";
        std::cin >> countdown_goal;
        if (countdown_goal == 0)
        {
