...
bool callback(example_ros_service::ExampleServiceMsgRequest& request, example_ros_service::ExampleServiceMsgResponse& response)
{
    ...
    string in_name(request.name); //conver this to a C++-class string ,so can use member funcs//cout<<"in_name:"<<in_name<<endl;
    response.on_the_list = false;
    //here is dumb way to access a stupid database
    //hey:this example is about services, not databses!
